#! /usr/bin/env python3

# Robótica Computacional
# Grado en Ingeniería Informática (Cuarto)
# Práctica 5:
#     Simulación de robots móviles holonómicos y no holonómicos.

#localizacion.py

import sys
from math import *
from robot import robot
import random
import numpy as np
import matplotlib.pyplot as plt
import time

# ******************************************************************************
# Declaración de funciones

def distancia(a,b):
  # Distancia entre dos puntos (admite poses)
  return np.linalg.norm(np.subtract(a[:2],b[:2]))

def angulo_rel(pose,p):
  # Diferencia angular entre una pose y un punto objetivo 'p'
  w = atan2(p[1]-pose[1],p[0]-pose[0])-pose[2]
  while w >  pi: w -= 2*pi
  while w < -pi: w += 2*pi
  return w

def mostrar(objetivos,ideal,trayectoria):
  # Mostrar objetivos y trayectoria:
  plt.figure('Trayectoria')
  plt.clf()
  plt.ion() # modo interactivo: show no bloqueante
  # Fijar los bordes del gráfico
  objT   = np.array(objetivos).T.tolist()
  trayT  = np.array(trayectoria).T.tolist()
  ideT   = np.array(ideal).T.tolist()
  bordes = [min(trayT[0]+objT[0]+ideT[0]),max(trayT[0]+objT[0]+ideT[0]),
            min(trayT[1]+objT[1]+ideT[1]),max(trayT[1]+objT[1]+ideT[1])]
  centro = [(bordes[0]+bordes[1])/2.,(bordes[2]+bordes[3])/2.]
  radio  = max(bordes[1]-bordes[0],bordes[3]-bordes[2])*.75
  plt.xlim(centro[0]-radio,centro[0]+radio)
  plt.ylim(centro[1]-radio,centro[1]+radio)
  # Representar objetivos y trayectoria
  idealT = np.array(ideal).T.tolist()
  plt.plot(idealT[0],idealT[1],'-g')
  plt.plot(trayectoria[0][0],trayectoria[0][1],'or')
  r = radio * .1
  for p in trayectoria:
    plt.plot([p[0],p[0]+r*cos(p[2])],[p[1],p[1]+r*sin(p[2])],'-r')
    #plt.plot(p[0],p[1],'or')
  objT   = np.array(objetivos).T.tolist()
  plt.plot(objT[0],objT[1],'-.o')
  plt.show()

def localizacion(balizas, real, ideal, centro, radio, mostrar=False):
  # Buscar la localización más probable del robot, a partir de su sistema
  # sensorial, dentro de una región cuadrada de centro "centro" y lado "2*radio".
  
  imagen = []
  pasos = 30
  step = (2.0 * radio) / pasos
  min_error = float('inf')
  best_x, best_y = centro[0], centro[1]  # Inicializar con el centro de búsqueda

  # 1. Leer sensores (Sin acceder a real.pose)
  mediciones_reales = real.senseDistance(balizas)
  orientacion_real = real.senseAngle(balizas)  # Obtener orientación desde sensores

  # 2. GRID SEARCH - Buscar mejor posición según sensores
  for i in range(pasos):
    fila = []
    y_test = (centro[1] - radio) + i * step
    
    for j in range(pasos):
      x_test = (centro[0] - radio) + j * step
      ideal.set(x_test, y_test, ideal.orientation)
      error_total = 0.0
      # Calcular error cuadrático acumulado
      for k, baliza in enumerate(balizas):
        dist_teorica = np.linalg.norm(np.subtract([x_test, y_test], baliza))
        dist_real = mediciones_reales[k]
        error_total += (dist_real - dist_teorica) ** 2

      if error_total < min_error:
        min_error = error_total
        best_x = x_test
        best_y = y_test

      fila.append(error_total)
    
    imagen.append(fila)

  # 3. APLICAR CORRECCIÓN - Actualizar ideal con la mejor posición encontrada
  ideal.set(best_x, best_y, orientacion_real)

  if mostrar:
    plt.figure('Localizacion')
    plt.clf()
    plt.ion() 
    plt.xlim(centro[0]-radio,centro[0]+radio)
    plt.ylim(centro[1]-radio,centro[1]+radio)
    imagen.reverse()
    plt.imshow(imagen,extent=[centro[0]-radio,centro[0]+radio,\
                              centro[1]-radio,centro[1]+radio])
    balT = np.array(balizas).T.tolist();
    plt.plot(balT[0],balT[1],'or',ms=10,label='Balizas')
    pose_final = ideal.pose()
    plt.plot(pose_final[0],pose_final[1],'D',c='#00ff00',ms=10,mew=2,label='Estimado')
    plt.plot(real.x, real.y, 'D',c='#1a2be3',ms=10,mew=2, label='Real')
    plt.legend()
    plt.show()
    plt.pause(0.01)

# ******************************************************************************

# Definición del robot:
P_INICIAL = [0.,4.,0.] # Pose inicial (posición y orientacion)
P_INICIAL_IDEAL = [2, 2, 0]  # Posición inicial errónea intencionadamente
V_LINEAL  = .7         # Velocidad lineal    (m/s)
V_ANGULAR = 140.       # Velocidad angular   (º/s)
FPS       = 10.        # Resolución temporal (fps)
MOSTRAR   = False   # Si se quiere gráficas de localización y trayectorias
UMBRAL_ERROR = 0.3          # Umbral para relocalizar (30cm)

HOLONOMICO = 1
GIROPARADO = 0
LONGITUD   = .2

# Definición de trayectorias:
trayectorias = [
    [[1,3]],
    [[0,2],[4,2]],
    [[2,4],[4,0],[0,0]],
    [[2,4],[2,0],[0,2],[4,2]],
    [[2+2*sin(.8*pi*i),2+2*cos(.8*pi*i)] for i in range(5)]
    ]

# Definición de los puntos objetivo:
if len(sys.argv)<2 or int(sys.argv[1])<0 or int(sys.argv[1])>=len(trayectorias):
  sys.exit(f"{sys.argv[0]} <indice entre 0 y {len(trayectorias)-1}>")
objetivos = trayectorias[int(sys.argv[1])]

# Balizas fijas
balizas = [[0,0], [4,0], [0,4], [4,4]]

# Definición de constantes:
EPSILON = .1                # Umbral de distancia
V = V_LINEAL/FPS            # Metros por fotograma
W = V_ANGULAR*pi/(180*FPS)  # Radianes por fotograma

ideal = robot()
ideal.set_noise(0,0,0)   # Ruido lineal / radial / de sensado
ideal.set(*P_INICIAL_IDEAL)     # operador 'splat'

real = robot()
real.set_noise(.01,.01,.1)  # Ruido lineal / radial / de sensado
real.set(*P_INICIAL)

tray_real = [real.pose()]     # Trayectoria seguida

tiempo  = 0.
espacio = 0.
random.seed(0)
#random.seed(time.time())
tic = time.time()

# Localización inicial
print("--- Iniciando Localización Global ---")

# Cálculo dinámico del centro y radio basado en balizas
bal_np = np.array(balizas)
min_x, max_x = np.min(bal_np[:, 0]), np.max(bal_np[:, 0])
min_y, max_y = np.min(bal_np[:, 1]), np.max(bal_np[:, 1])
centro_calc = [(min_x + max_x) / 2.0, (min_y + max_y) / 2.0]
radio_calc = max(max_x - min_x, max_y - min_y) / 2.0 + 1.5

localizacion(balizas, real, ideal, centro=centro_calc, radio=radio_calc, mostrar=MOSTRAR)

tray_ideal = [ideal.pose()]  # Trayectoria percibida

distanciaObjetivos = []
for punto in objetivos:
  while distancia(tray_ideal[-1],punto) > EPSILON and len(tray_ideal) <= 1000:
    pose = ideal.pose()

    w = angulo_rel(pose,punto)
    if w > W:  w =  W
    if w < -W: w = -W
    v = distancia(pose,punto)
    if (v > V): v = V
    if (v < 0): v = 0

    if HOLONOMICO:
      if GIROPARADO and abs(w) > .01:
        v = 0
      ideal.move(w,v)
      real.move(w,v)
    else:
      ideal.move_triciclo(w,v,LONGITUD)
      real.move_triciclo(w,v,LONGITUD)
    tray_real.append(real.pose())

    # Decidir nueva localización ⇒ nuevo ideal
    # Calcular desviación instantánea
    dist_error = distancia(real.pose(), ideal.pose())
    
    if dist_error > UMBRAL_ERROR:
        # Relocalizar en entorno global
        print(f"¡Corrección! Error: {dist_error:.3f}m > {UMBRAL_ERROR}m - Relocalizando...")
        localizacion(balizas, real, ideal, centro=centro_calc, radio=radio_calc, mostrar=MOSTRAR)

    tray_ideal.append(ideal.pose())

    if MOSTRAR:
      mostrar(objetivos, tray_ideal, tray_real)
      plt.pause(0.01)

    espacio += v
    tiempo  += 1
  # Antes de pasar a un nuevo punto apuntamos distancia a este objetivo
  distanciaObjetivos.append(distancia(tray_real[-1], punto))


toc = time.time()

# ******************************************************************************
# SALIDA DE RESULTADOS
# ******************************************************************************

# Calcular desviación acumulada
desviacion = 0.0
for i in range(min(len(tray_real), len(tray_ideal))):
    d = np.linalg.norm(np.subtract(tray_real[i][:2], tray_ideal[i][:2]))
    desviacion += d

print("\n" + "="*60)
print("Tiempo real invertido: {:.3f}s".format(toc-tic))
print("Recorrido: {:.3f}m / {:.1f}s".format(espacio, tiempo/FPS))
print("Desviación de las trayectorias: {:.3f}".format(desviacion))
print("   (Promedio de error por paso: {:.4f} m)".format(desviacion/len(tray_real)))
print("Suma de distancias a objetivos: {:.3f}m".format(np.sum(distanciaObjetivos)))
print("Distancia real al objetivo final: {:.3f}m".format(distanciaObjetivos[-1]))

# Distancias finales a las balizas
pose_final_real = real.pose()
print("\nDistancia final del robot a las balizas:")
for i, b in enumerate(balizas):
    d_baliza = distancia(pose_final_real, b)
    print("   - Baliza {0} {1}: {2:.4f} metros".format(i, b, d_baliza))

print("="*60)

if len(tray_ideal) > 1000:
  print("<!> AVISO: Trayectoria muy larga, quizás no alcanzó el objetivo.")

print("\nResumen: {:.3f} {:.3f} {:.3f}".format(toc-tic, desviacion, np.sum(distanciaObjetivos)))

if MOSTRAR:
  mostrar(objetivos, tray_ideal, tray_real)
  print("\nPresiona Enter para cerrar las gráficas...")
  input()